...
export default function FormularioAdopcion({ onSubmit, petId = null, step = "1" }) {
  const { show } = useToast();
  ...
  useEffect(() => {
    if (petId) {
      setForm((s) => ({ ...s, mascotaID: String(petId) }));
    }
  }, [petId]);
  ...
  const validate = () => {
    ...
    if (!/^\d{1,2}$/.test(String(form.edad))) {
      setErrMsg(show({ title: "Error", message: "Edad inválida", variant: "danger" }));
      return false;
    }
    if (!/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(form.email)) {
      setErrMsg(show({ title: "Error", message: "Correo inválido", variant: "danger" }));
      return false;
    }
    if (!/^[0-9+\-\s()]{8,}$/.test(form.telefono)) {
      setErrMsg(show({ title: "Error", message: "Teléfono inválido", variant: "danger" }));
      return false;
    }
    setErrMsg("");
    return true;
  ...
  const inputClass = "w-full rounded-md border px-4 py-3 focus:outline-none focus:ring-2 focus:ring-offset-1";
  const labelClass = "block text-sm font-semibold mb-1";
  const sectionClass = "bg-white rounded-xl shadow-sm p-5 md:p-6 border";
      ...
      <div className="mx-auto max-w-6xl px-4 pt-4">
        <div className="flex items-center justify-between mb-4 text-sm text-[#6b7076]">
          <nav className="flex items-center gap-2">
            <Link href="/adoptante">Panel</Link>
            <span aria-hidden>/</span>
            {petId ? <Link href={`/mascotas/${petId}`}>Perfil</Link> : <span>Perfil</span>}
            <span aria-hidden>/</span>
            <span className="font-medium">Solicitud de Adopción</span>
          </nav>

          {petId && (
            <Link href={`/mascotas/${petId}`} className="rounded-xl px-4 py-2 bg-[#fceae0] text-[#9f5b53] hover:bg-[#f8dfd2]">
              <span>←</span> Volver
            </Link>
          )}
        </div>
        ...
        <form onSubmit={submit} className="space-y-6">
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              style={{ background: PALETTE.accent, color: "white", opacity: loading ? 0.7 : 1 }}
